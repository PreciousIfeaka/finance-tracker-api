...
class BudgetServiceTest {
    ...
    void createBudget_ShouldCreateBudget_WhenRulesAreMet() {
        CreateBudgetRequestDto dto = new CreateBudgetRequestDto(BigDecimal.valueOf(200), ExpenseCategory.FOOD, false);
        YearMonth currentMonth = YearMonth.now();

        when(userService.getAuthenticatedUser()).thenReturn(mockUser);
        
        Income income = new Income();
        when(incomeRepository.findAllByUserIdAndMonthOrderByTransactionDateTimeDesc(eq(mockUser.getId()), eq(currentMonth)))
        ...
        when(budgetRepository.getTotalBudgetByMonth(mockUser.getId(), currentMonth))
                .thenReturn(BigDecimal.valueOf(0)); // Note: Mockito defaults to 0 if not mocked, but we should be explicit, however the repo might return null in real life. Assuming it returns 0 or we handle it in service. Let's return ZERO.
        when(budgetRepository.getTotalBudgetByMonth(mockUser.getId(), currentMonth)).thenReturn(BigDecimal.ZERO);

        when(budgetRepository.findByUserIdAndMonthAndCategory(mockUser.getId(), currentMonth, ExpenseCategory.FOOD))
                .thenReturn(Optional.empty());
        when(budgetRepository.findByUserIdAndMonthAndCategory(mockUser.getId(), currentMonth, ExpenseCategory.all))
                .thenReturn(Optional.empty());

        Page<Budget> emptyPage = new PageImpl<>(List.of());
        when(budgetRepository.findAllByUserIdAndMonthOrderByCreatedAtDesc(eq(mockUser.getId()), eq(currentMonth), any(PageRequest.class)))
                .thenReturn(emptyPage);
        // budgetRepository.getTotalBudgetByMonth is already mocked to ZERO

        when(budgetRepository.findByUserIdAndAmountAndCategory(mockUser.getId(), BigDecimal.valueOf(200), ExpenseCategory.FOOD))
                .thenReturn(Optional.empty());

        when(expenseRepository.sumExpenseByUserIdAndMonthAndCategory(mockUser.getId(), currentMonth, ExpenseCategory.FOOD))
    ...
    void createBudget_ShouldThrowBadRequest_WhenNoIncome() {
        CreateBudgetRequestDto dto = new CreateBudgetRequestDto(BigDecimal.valueOf(200), ExpenseCategory.FOOD, false);
        when(userService.getAuthenticatedUser()).thenReturn(mockUser);
        when(incomeRepository.findAllByUserIdAndMonthOrderByTransactionDateTimeDesc(eq(mockUser.getId()), any()))
                .thenReturn(List.of());
    ...
    void createBudget_ShouldThrowBadRequest_WhenBudgetExceedsIncome() {
        CreateBudgetRequestDto dto = new CreateBudgetRequestDto(BigDecimal.valueOf(2000), ExpenseCategory.FOOD, false);
        when(userService.getAuthenticatedUser()).thenReturn(mockUser);
        when(incomeRepository.findAllByUserIdAndMonthOrderByTransactionDateTimeDesc(eq(mockUser.getId()), any()))
                .thenReturn(List.of(new Income()));
        
        when(incomeRepository.getTotalIncomeByMonth(eq(mockUser.getId()), any())).thenReturn(BigDecimal.valueOf(1000));
    ...
    void getBudget_ShouldReturnBudget() {
        when(userService.getAuthenticatedUser()).thenReturn(mockUser);
        when(budgetRepository.findByIdAndUserId(mockBudget.getId(), mockUser.getId())).thenReturn(Optional.of(mockBudget));
    ...
    void updateBudget_ShouldUpdateWhenValid() {
        UpdateBudgetRequestDto dto = new UpdateBudgetRequestDto(BigDecimal.valueOf(600), ExpenseCategory.BILLS, true);
        ...
        when(expenseRepository.sumExpenseByUserIdAndMonthAndCategory(mockUser.getId(), mockBudget.getMonth(), ExpenseCategory.BILLS))
    ...
    void getAllBudgets_ShouldReturnPagedResponse() {
        ...
        assertEquals("Success", result.getStatus());
        assertEquals(1, result.getData().budgets().getTotalElements());
